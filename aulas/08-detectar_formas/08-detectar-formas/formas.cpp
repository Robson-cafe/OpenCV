...
void Formas::desenhar_nas_formas(){
  ...
  for (size_t i = 0; i < contornos.size(); ++i) {
   ...
   //cv::drawContours( img, lados, i, cv::Scalar(147,20,255), 2 );
   ...
   cv::putText( img, nome_da_forma, cv::Point( retangulo[i].x, retangulo[i].y ), cv::QT_FONT_NORMAL, 1, cv::Scalar(0,0,0), 2 );

   //cv::drawContours( img, contornos, i, cv::Scalar(147,20,255), 2 );
